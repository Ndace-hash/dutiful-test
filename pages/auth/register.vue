    ...
    <div class="flex py-12 items-center justify-center w- px-4">
        <form class=" min-h-[200px] w-full max-w-[400px] mb-4" ref="form" @submit.prevent="signUp">
            ...
            <div class="flex gap-4 w-full">
                <div :class="['form-group flex items-center justify-center  mb-6  rounded-lg py-4 px-3 relative cursor-pointer', role === 'user' ? 'border border-primary text-primary' : 'text-[#B1BDCA]']"
                    @click="role = 'user'">
                ...
                <div :class="['form-group flex items-center justify-center  mb-6 rounded-lg py-4 px-3 relative cursor-pointer', role === 'service' ? 'border border-primary text-primary' : 'text-[#B1BDCA]']"
                    @click="role = 'service'">
            ...
            <div class="form-group flex flex-col gap-1 mb-6">
                ...
                <input type="text" name="fullName" id="fullName"
                    class="border border-[#E7EAF1] bg-[#F8FAFD] py-2 px-3 focus:outline-primary" v-model="formData.name">
            </div>
            <div class="form-group flex flex-col gap-1 mb-6">
                <label for="email" class="text-primary">Email</label>
                <input type="email" name="email" id="email"
                    class="border border-[#E7EAF1] bg-[#F8FAFD] py-2 px-3 focus:outline-primary" v-model="formData.email">
            </div>
            <div class="form-group flex flex-col gap-1 mb-6">
                <label for="phone" class="text-primary">Phone number</label>
                <input type="text" name="phone" id="phone"
                    class="border border-[#E7EAF1] bg-[#F8FAFD] py-2 px-3 focus:outline-primary" v-model="formData.phone">
            </div>
            <div class="form-group flex flex-col gap-1 mb-6">
                <label for="password" class="text-primary">Password</label>
                <input type="password" name="password" id="password"
                    class="border border-[#E7EAF1] bg-[#F8FAFD] py-2 px-3 focus:outline-primary"
            ...
            <div class="form-group flex flex-col gap-1 mb-6">
                ...
                <div class="mt-2">
                    ...
                    <label for="tnc"> I agree to Dutiful's <NuxtLink to="/" class="text-primary underline">terms and
                            conditions</NuxtLink></label>
                </div>
            </div>
            <p class="text-white bg-green-400 text-center font-bold text-lg py-4 px-6 rounded-lg mb-6 shadow-md"
                v-if="success">Registration successful!</p>
            <div class="flex flex-col gap-2 mb-6" v-if="error">

                <p class="text-white bg-red-300 text-center font-bold text-sm py-4 px-6 rounded-lg shadow-md"
                    v-for="err in error">{{ err[0] }}</p>
            </div>
            <input type="submit" value="login"
                class="bg-primary w-full text-white capitalize text-base py-4 rounded-lg mb-6 font-semibold"
                :disabled="!agreement">
            <p class="text-secondary text-center">Already have an account? <NuxtLink to="/auth/login"
                    class="text-primary font-semibold">Login</NuxtLink>
            </p>
        </form>
    </div>
</template>
